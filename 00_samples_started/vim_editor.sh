...
#       vim 编辑器
# ======================================================================================
# vim 是linux下强大的编辑器,  可以和 emacs 相媲美，使用emacs 并没有使用 vi 进行编辑来得高效
# 如果是初学 vi，运行一下 vimtutor 是个聪明的决定
# 如果你的系统环境不是中文，而你想使用中文的 vimtutor，就运行
# $ vimtutor zh
# --------------------------------------------------------------------------------------
...
插入模式
# 可以输入文本，在正常模式下，按 i、a、o、Insert 等都可以进入插入模式
...
vim --remote file   用已有的 vim 进程打开指定的文件,  如果你不想启用多个 vim 会话，这个很有用
                    ...
                    如果你已经有一个 gvim 在运行了
                    你可以用 gvim --remote file 在已有的 gvim 中打开文件
...
i           在光标前插入；一个小技巧：按8，再按i，进入插入模式，输入=， 按esc进入命令模式，就会出现8个=
            这在插入分割线时非常有用，如 30i+<esc> 就插入了 36个+ 组成的分割线
I           在当前行第一个非空字符前插入
gI          在当前行第一列插入
a           在光标后插入
A           在当前行最后插入
...
如果是 Windows 文件格式（即换行符有两个字节），字数的统计方法为： 字符数 - 行数 * 2
...
:close      关闭当前窗口，CTRL-W c 能实现同样的功能(象 :q :x同样工作)
...
# 12.2 文件的编码
# --------------------------------------------------------------------------------------
# 在vimrc中添加 set fileencoding=ucs-bom,utf-8,cp936  vim会根据要打开的文件选择合适的编码
# 注意：编码之间不要留空格，cp936对应于gbk编码；ucs-bom对应于windows下的文件格式
# 让vim 正确处理文件格式和文件编码，有赖于 ~/.vimrc 的正确配置
...
# 三种格式的区别主要在于回车键的编码：dos 下是回车加换行；unix 下只有 换行符；mac 下只有回车符；
# 在 vimrc 中添加 set fileformats=unix,dos,mac  让vim自动识别文件格式
...
# 13.4 gtags
# --------------------------------------------------------------------------------------
# Gtags 综合了ctags和cscope的功能，使用Gtags之前，你需要安装GNU Gtags,  然后在工程目录运行 gtags

:Gtags funcname         定位到 funcname 的定义处
:Gtags -r funcname      查询 funcname被引用的地方
:Gtags -s symbol        定位 symbol 出现的地方
:Gtags -g string        Goto string 出现的地方
:Gtags -gi string       Goto string 出现的地方，忽略大小写
...
# 13.5 编译
# --------------------------------------------------------------------------------------
# vim提供了:make 来编译程序，默认调用的是 make， 如果你当前目录下有 makefile，简单地 :make 即可
# 如果你没有 make 程序，你可以通过配置 makeprg 选项来更改 make 调用的程序
# 如果你只有一个 abc.java 文件，你可以这样设置：

set makeprg=javac\ abc.java

# 然后 :make 即可, 如果程序有错，可以通过 quickfix 窗口查看错误
# 不过如果要正确定位错误，需要设置好 errorformat 让 vim 识别错误信息,  如：

:setl efm=%A%f:%l:\ %m,%-Z%p^,%-C%.%#

# %f 表示文件名，%l 表示行号，%m 表示错误信息，其它的还不能理解
...
Shift-Right 右移一个单词
@           重复上一次的冒号命令
q           正常模式下，q 再按 : 打开命令行历史缓冲区，可以像编辑文件一样编辑命令
q/和q?      可以打开查找历史记录
...
:m,nw !cmd  将文件的 m 行到n行之间的内容做为命令输入执行命令
...
# 15.4 一些小功能
# --------------------------------------------------------------------------------------
C-r = 简单计算器:    在插入模式下，输入C-r =，然后输入表达式，就能在 光标处得到计算结果